...
simulation_index = 1:nrow(simulation_grid)

my_files = list.files(paste0(getwd(),"/results") ) 

if(length(my_files)>0 ){
  # Find all numbers in the text
  # Find all entries that start with "result_knn_"
  matches <- grep("^result_knn_", my_files, value = TRUE)
...
for(simulation_i in simulation_index ){
  cmd = paste("sbatch","  Multiple_jobs_knn.sh ",simulation_i,sep="")
  print(cmd)
  system(cmd)
}
